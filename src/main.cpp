...
void init() {
  ...
  glGenerateMipmap(GL_TEXTURE_2D); // try also activating mipmaps for the second
                                   // texture object
  textureSelect = 0;
  ...
  if (action == GLFW_PRESS || action == GLFW_REPEAT) {
    ...
    case GLFW_KEY_H:
      ...
      std::cout << "Click the left mouse button to toggle the render mode "
                ...
                << std::endl;
      std::cout
          << "Click the right mouse button to toggle the object type (sphere)"
          << std::endl;
...
void create_sphere() {
  ...
  for (int i = 0; i <= STACK_COUNT; ++i) {
    ...
    for (int j = 0; j <= SECTOR_COUNT; ++j) {
      ...
      v = 0.5 - asin(y) / M_PI;
      tmp_sphere_tex_coords.emplace_back((float)j / SECTOR_COUNT,
                                         (float)i / STACK_COUNT);
    }
  }
