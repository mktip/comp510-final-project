...
inline void update_shader_lighting_parameters_sphere(
...
public:
  Sphere(GLfloat r, vector<color4> m, GLfloat s) {
    radius = r;
    materials = m;
    Shininess = s;
    create_sphere();
  ...
  void draw(GLuint program, Light &light) {
    update_shader_lighting_parameters_sphere(
        materials[0], materials[1], materials[2], Shininess, light, program);
